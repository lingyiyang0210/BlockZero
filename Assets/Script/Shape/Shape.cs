...
public class Shape : MonoBehaviour, IPointerClickHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IPointerDownHandler
{
    ...
    private Vector2 offset = new Vector2(0f, 150f);
    ...
    public void Awake()
    {
        ...
        _originalPivot = _transform.pivot;
    ...
    public bool IsShapeActive() => gameObject.activeSelf && _shapeActive;

    public bool IsOnStartPosition() => Vector3.Distance(_transform.localPosition, _startPosition) < 0.1f;
    ...
    public void DeactivateShape()
    {
        if (_shapeActive)
        {
            foreach (var square in _currentShape)
            {
                if (square.activeSelf) square?.GetComponent<ShapeSquare>().DeactivateShape();
            }
        }
        _shapeActive = false;
    }
    ...
    public void ActivateShape()
    {
        ...
        foreach (var square in _currentShape)
        {
            if (square.activeSelf) square?.GetComponent<ShapeSquare>().ActivateShape();
        }
    }
        ...
        for (var row = 0; row < shapeData.rows; row++)
        {
            for (var column = 0; column < shapeData.columns; column++)
            {
                if (shapeData.board[row].column[column])
                {
                    ...
                    _currentShape[currentIndexInList].transform.localScale = Vector3.one;
    ...
    private float GetXPositionForShapeSquare(ShapeData shapeData, int column, Vector2 moveDistance)
    {
        ...
        if (shapeData.columns > 1) //vertical position calculation
        {
            if (shapeData.columns % 2 != 0)
            {
                ...
                if (column < middleSquareIndex) //move it on the negative position
                {
    ...
    {
        if (_shapeDraggable == false) return;
        ...
        _dragOffset = ((Vector2)_transform.localPosition + offset) - mousePos;
    }
